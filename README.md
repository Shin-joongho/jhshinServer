...
논리 6코어 환경에서, 유출량이 비슷해도 **룸 크기에 따라 지연이 크게 갈리는** 구간을 확인했습니다.
...
다만 이후 **논리 20코어 환경에서 같은 비교를 재측정했을 때는 이 격차가 나타나지 않았습니다.** 머신·툴체인·코드가 동시에 바뀌어 원인을 분리하지 못했고, 그 과정을 [`RESULTS.md` 14절](TestClient/RESULTS.md)에 그대로 남겼습니다. **이 특성이 나타나는 조건은 아직 특정하지 못했습니다.**
...
유출량이 비슷해도 룸 크기에 따라 지연이 크게 달라졌습니다. (논리 6코어)
...
브로드캐스트는 룸 멤버를 한 스레드에서 동기로 순회하므로, 룸이 커질수록 순회 뒤쪽 멤버가 밀립니다.

### 재현되지 않은 측정

이후 논리 20코어에서 같은 비교를 다시 돌렸을 때는 **격차가 나타나지 않았습니다** — 팬아웃 10과 20에서 p50이 0.244 ms와 0.253 ms로 사실상 같습니다. affinity로 6코어에 묶어도 마찬가지였습니다.

머신·툴체인·코드가 동시에 바뀌어 **원인을 분리하지 못했습니다.** 위 특성이 성립하는 조건을 아직 특정하지 못했다는 뜻이며, 재현 시도와 분리하지 못한 변수를 [`RESULTS.md` 14절](TestClient/RESULTS.md)에 그대로 기록했습니다.

`Room::BroadCast`가 한 스레드에서 동기로 순회한다는 **구조적 사실은 그대로**입니다.
...
상세한 측정 과정은 [`TestClient/RESULTS.md`](TestClient/RESULTS.md)의 14개 절에 기록했습니다.
